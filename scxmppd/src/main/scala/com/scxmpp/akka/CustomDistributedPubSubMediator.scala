...
import akka.routing.{Router => AkkaRouter}
...
object CustomDistributedPubSubMediator {

  /**
   * Scala API: Factory method for `CustomDistributedPubSubMediator` [[akka.actor.Props]].
   */
  def props(
    role: Option[String],
    ...
    Props(classOf[CustomDistributedPubSubMediator], role, routingLogic, gossipInterval, removedTimeToLive, maxDeltaElements)

  /**
   * Java API: Factory method for `CustomDistributedPubSubMediator` [[akka.actor.Props]].
   */
  def props(
    role: String,
...
class CustomDistributedPubSubMediator(
  ...
  import CustomDistributedPubSubMediator._
  import CustomDistributedPubSubMediator.Internal._
  ...
  def receive = {
    ...
    case Send(path, msg, localAffinity) =>
      ...
      if (routees.nonEmpty)
        AkkaRouter(routingLogic, routees).route(wrapIfNeeded(msg), sender())
...
/**
 * Extension that starts a [[CustomDistributedPubSubMediator]] actor
 * with settings defined in config section `akka.contrib.cluster.pub-sub`.
 */
object CustomDistributedPubSubExtension extends ExtensionId[CustomDistributedPubSubExtension] with ExtensionIdProvider {
  override def get(system: ActorSystem): CustomDistributedPubSubExtension = super.get(system)

  override def lookup = CustomDistributedPubSubExtension

  override def createExtension(system: ExtendedActorSystem): CustomDistributedPubSubExtension =
    new CustomDistributedPubSubExtension(system)
}

class CustomDistributedPubSubExtension(system: ExtendedActorSystem) extends Extension {
  ...
  /**
   * The [[CustomDistributedPubSubMediator]]
   */
  val mediator: ActorRef = {
    if (isTerminated)
      ...
      system.actorOf(CustomDistributedPubSubMediator.props(role, routingLogic, gossipInterval, removedTimeToLive, maxDeltaElements),
        name)
    }
  }
}
